<?php
        ...
        if (count($user) !== 0) {
            if ($user['login'] == Request::post('login')) {
                if (trim($user['password']) == Security::encryptPassword(Request::post('password'))) {
                    ...
                    $login_error = __('Wrong <b>login</b> or <b>password</b>', 'users');
                }
            } 
        } else {
            $login_error = __('Wrong <b>login</b> or <b>password</b>', 'users');
    ...
    if (Request::post('reset_password_submit')) {
        ...
        if (count($user) !== 0) {
            ...
            if ($user['login'] == Request::post('login')) {
                ...
                $reset_password_error = __('This user does not exist', 'users');
            }
        } else {
            Notification::setNow('reset_password_error', 'reset_password_error');
            $reset_password_error = __('This user does not exist', 'users');
    ...
    if ($is_admin) {        
        ...
        require('themes/default/admin.template.php');
        ...
        require('themes/default/login.template.php');
