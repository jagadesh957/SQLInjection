<?php defined('MONSTRA_ACCESS') or die('No direct script access.');
    ...
    class Plugin {
        ...
        protected function __construct() {            
            ...
            // Backend or Frontend
            if (BACKEND) $admin_path = '../'; else $admin_path = '';
            ...
            foreach ($records as $record) {
                if ($record['status'] == 'active') {
                    include_once $admin_path . $record['location'];
                }
            }
        }
        ...
        public static function admin($plug, $alt_folder = null) {
            ...
            if (BACKEND) {
                ...
                $path = $admin_path . 'plugins' . DS . $folder . DS . $plug . $ext;
        ...
        protected function __construct($locale) {    
            ...
            if ( ! $lang_table) {  
                ...
                foreach ($records as $record) {  
                    ...
                    if (is_dir($admin_path.dirname($record['location']). '/' . 'languages')) {                    
                        
                        // Init var
                        $t = array();            
                                    
                        // Check lang file            
                        if (file_exists($admin_path.dirname($record['location']). '/' . 'languages' . DS . $locale . '.lang.php')) {

                            // Merge the language strings into the sub table
                            $t = array_merge($t, include $admin_path.dirname($record['location']). '/' . 'languages' . DS . $locale .'.lang.php');                                                    
        ...
        public static function load() {            
            ...
            if (count(Stylesheet::$stylesheets) > 0) {
                ...
                foreach ($stylesheets as $stylesheet) {
                    if ((file_exists($stylesheet['file'])) and (($stylesheet['load'] == 'backend') or ($stylesheet['load'] == 'both')) ) {
                        if ( ! file_exists($backend_site_css_path) or filemtime($stylesheet['file']) > filemtime($backend_site_css_path)) {
                ...
                if ($backend_regenerate) {
                    foreach ($stylesheets as $stylesheet) {                        
                        if ((file_exists($stylesheet['file'])) and (($stylesheet['load'] == 'backend') or ($stylesheet['load'] == 'both')) ) {
                            $backend_buffer .= file_get_contents($stylesheet['file']);                            
                        }
                    }
                ...
                foreach ($stylesheets as $stylesheet) {                    
                    if ((file_exists($stylesheet['file'])) and (($stylesheet['load'] == 'frontend') or ($stylesheet['load'] == 'both')) ) {
                        if ( ! file_exists($frontend_site_css_path) or filemtime($stylesheet['file']) > filemtime($frontend_site_css_path)) {
                ...
                if ($frontend_regenerate) {
                    foreach ($stylesheets as $stylesheet) {                        
                        if ((file_exists($stylesheet['file'])) and (($stylesheet['load'] == 'frontend') or ($stylesheet['load'] == 'both')) ) {
                            $frontend_buffer .= file_get_contents($stylesheet['file']);
                        }
                    }
        ...
        public static function parseVariables($frontend_buffer) {
            return str_replace(array('@site_url',
                               ...
                               array(Option::get('siteurl'),
                                     Option::get('siteurl').'public/themes/'.Option::get('theme_name')),
                               $frontend_buffer);
        }
        ...
        public static function load() {
            ...
            if (count(Javascript::$javascripts) > 0) {
                ...
                foreach ($javascripts as $javascript) {
                    if ((file_exists($javascript['file'])) and (($javascript['load'] == 'backend') or ($javascript['load'] == 'both')) ) {
                        if ( ! file_exists($backend_site_js_path) or filemtime($javascript['file']) > filemtime($backend_site_js_path)) {
                            $backend_regenerate = true;
                            break;
                        }
                    } 
                ...
                if ($backend_regenerate) {
                    foreach ($javascripts as $javascript) {                        
                        if ((file_exists($javascript['file'])) and (($javascript['load'] == 'backend') or ($javascript['load'] == 'both')) ) {
                            $backend_buffer .= file_get_contents($javascript['file']);
                        }
                    }
                ...
                foreach ($javascripts as $javascript) {                    
                        if ((file_exists($javascript['file'])) and (($javascript['load'] == 'frontend') or ($javascript['load'] == 'both')) ) {
                        if ( ! file_exists($frontend_site_js_path) or filemtime($javascript['file']) > filemtime($frontend_site_js_path)) {
                            $frontend_regenerate = true;
                            break;
                        }
                    } 
                ...
                if ($frontend_regenerate) {
                    foreach ($javascripts as $javascript) {                        
                        if ((file_exists($javascript['file'])) and (($javascript['load'] == 'frontend') or ($javascript['load'] == 'both')) ) {
                            $frontend_buffer .= file_get_contents($javascript['file']);
                        }
                    }
